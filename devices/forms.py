...
class PositionForm(ModelForm):
    ...
    def clean_device(self):
        device_sn = self.cleaned_data.get('device').strip()
        if not device_sn:
            raise forms.ValidationError('设备sn不能为空，请扫码/输入')

        try:
            return Device.objects.get(sn=device_sn)
        except Device.DoesNotExist:
            raise forms.ValidationError(f'设备SN {device_sn} 不存在，请重新扫码/输入')

    # 校验 and 处理表单数据，返回符合数据库要求的对象
    def clean_owner(self):
        owner_number = self.cleaned_data.get('owner').strip()
        if not owner_number:
            raise forms.ValidationError('负责人工号不能为空，请扫码/输入')

        try:
            return Employee.objects.get(number=owner_number)
